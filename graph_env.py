...
class HolEnv():
    def __init__(self, goal):
        ...
        self.current_goals = nodes_list(self.graph)
        ...
        # environment history only needs to be list of current goals per step, as when combined with the action can fully
        # reconstruct proof trace
        self.history = [self.current_goals]
    ...
    def step(self, action):
        ...
        goal_id, tactic = action

        goal_node = self.current_goals[goal_id]
        print (goal_node)

        target = goal_node.goal[0]["plain"]
        ...
        if d == "unexpected":
            ...
            if [goal_node.goal] != d:
                ...
                if d == []:
                    ...
                    #propagate up through current state
                    goal_node.prop_proved()
                    self.current_goals = nodes_list(self.graph)
                    self.history.append(self.current_goals)

                else:
                    reward = 0.1
                    #add subgoals to current state
                    print (d, goal_node.goal)
