...
with open("transformer_vocab.pk", "rb") as f:
    vocab = pickle.load(f)
...
class TransformerVanilla(Agent):
    ...
    def save_replays(self):
        with open("transformer_agent_replays.json", "w") as f:
...
def run_experiment():
    try:
        agent = TransformerVanilla(tactic_pool, replay_dir="transformer_agent_replays.json")
        agent.load()
